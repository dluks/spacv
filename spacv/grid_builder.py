...
from .utils import convert_geodataframe, convert_numpy, geometry_to_2d

__all__ = ["construct_blocks", "construct_square_grid", "construct_hex_grid"]
...
def construct_blocks(
    ...
    if method == "unique":
        ...
        if shape != "square":
            raise Exception(
                "systematic grid assignment method does not work for irregular grids."
            )
        ...
        grid["grid_id"] = assign_optimized_random(
            grid, XYs, data, n_groups, n_sims, distance_metric
        )
    else:
        raise ValueError(
            "Method not recognised. Choose between: unique, systematic, random or optimized_random."
        )
    return grid
...
def construct_square_grid(XYs, tiles_x, tiles_y):
    ...
    bottom_left = np.add(minx, np.multiply(rows, dx)), np.add(
        miny, np.multiply(columns, dy)
    )
    bottom_right = np.add(minx, np.multiply(rows + 1, dx)), np.add(
        miny, np.multiply(columns, dy)
    )
    top_right = np.add(minx, np.multiply(rows + 1, dx)), np.add(
        miny, np.multiply(columns + 1, dy)
    )
    top_left = np.add(minx, np.multiply(rows, dx)), np.add(
        miny, np.multiply(columns + 1, dy)
    )

    polys = np.vstack([bottom_left, bottom_right, top_right, top_left]).reshape(
        4, 2, -1
    )
...
def assign_systematic(grid, tiles_x, tiles_y, direction="diagonal"):
    ...
    if direction == "diagonal":
        diags = [sys_matrix.diagonal(i) for i in range(width - 1, -length, -1)]
    elif direction == "anti":
        diags = [sys_matrix[::-1, :].diagonal(i) for i in range(-length + 1, width)]
    else:
        raise ValueError(
            "Direction of systematic pattern not recognised. Choose between: diagonal or anti."
        )

    # Construct lookup between diagonal element indices and grid dataframe
    systematic_lookup = dict(
        [tuple([element, key]) for key, diag in enumerate(diags) for element in diag]
    )
...
def assign_optimized_random(
    grid, XYs, data, n_groups=5, n_sims=10, distance_metric="euclidean"
):
    """
    ...
    for sim in range(n_sims):
        ...
        sse = sum(sum((X_grid_means - Xbar) ** 2))
