...
from .utils import geometry_to_2d, load_custom_polygon

__all__ = ["HBLOCK", "SKCV", "RepeatedSKCV", "UserDefinedSCV"]
...
class SKCV(BaseSpatialCV):
    ...
    def __init__(self, n_splits=10, buffer_radius=0, random_state=None):
    ...
    def _iter_test_indices(self, XYs):
        ...
        lattice = any(XYs.geom_type == "Polygon") or any(
            XYs.geom_type == "MultiPolygon"
        )
        ...
        if sloo:
            ...
            km_skcv = MiniBatchKMeans(
                n_clusters=self.n_splits, random_state=self.random_state
            )
            ...
            if check_fold_n.any():
                warn = "{} folds contain less than three points and do not form polygons.".format(
                    check_fold_n.sum()
                )
                warnings.warn(warn)
            indices_from_folds = [np.argwhere(labels == i).reshape(-1) for i in uniques]
        for fold_indices in indices_from_folds:
            if sloo:
                ...
                test_indices = np.array(fold_indices)
                fold_polygon = XYs.loc[test_indices].unary_union.buffer(
                    self.buffer_radius
                )
            else:  # skcv
                test_indices = np.array(fold_indices)
                fold_polygon = XYs.loc[test_indices].unary_union.convex_hull.buffer(
                    self.buffer_radius
                )

            test_indices, train_exclude = super()._remove_buffered_indices(
                XYs, test_indices, self.buffer_radius, fold_polygon
            )
            yield test_indices, train_exclude
...
class RepeatedSKCV(SKCV):
    ...
    **cvargs : additional params
    ...
    def __init__(self, n_repeats=10, n_splits=10, random_state=None, **cvargs):
...
class UserDefinedSCV(BaseSpatialCV):
    ...
    def __init__(self, custom_polygons, buffer_radius=0, distance_metric="euclidean"):
        self.buffer_radius = buffer_radius
